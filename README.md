...
3. Upload all files **and keep the folder structure**: the `assets` folder must contain `css/`, `js/`, and `img/` with the right files inside. If you drag-and-drop, make sure to upload files into paths like `assets/css/styles.css`, not everything at the root.
...
**If the published site has no color or design:** GitHub may not have preserved the `assets` folder structure when you uploaded. See **[GITHUB-PAGES-FIX.md](GITHUB-PAGES-FIX.md)** for step-by-step instructions to add the CSS, JS, and images in the correct paths.
